...
(function () {
  ...
  class HeroProvider implements EIP1193Provider {
    ...
    private readonly _isMetaMask = true; // For compatibility with dApps that check for MetaMask
    ...
    private handleUnhandledRejection(event: PromiseRejectionEvent): void {
      if (event.reason?.stack?.includes('HeroProvider')) {
        console.error(
          'Hero Wallet Provider Unhandled Rejection:',
          event.reason
        );
    ...
    // Legacy provider compatibility - writable for compatibility with old dApps
    private _autoRefreshOnNetworkChange = false;

    get autoRefreshOnNetworkChange(): boolean {
      return this._autoRefreshOnNetworkChange;
    }

    set autoRefreshOnNetworkChange(value: boolean) {
      console.warn(
        '⚠️ autoRefreshOnNetworkChange is deprecated and has no effect'
      );
      this._autoRefreshOnNetworkChange = value;
    }
    ...
    enable(): Promise<string[]> {
      ...
      return this.request({ method: 'eth_requestAccounts' }) as Promise<
        string[]
      >;
    }
    ...
    send(methodOrPayload: unknown, callbackOrParams?: unknown): unknown {
      ...
      if (typeof methodOrPayload === 'string') {
        ...
        if (callbackOrParams && typeof callbackOrParams === 'function') {
          result
            .then((res) =>
              (callbackOrParams as Function)(null, { result: res })
            )
    ...
    sendAsync(
      ...
      if (typeof callback !== 'function') {
        throw this.createError(
          4200,
          'Callback function required for sendAsync'
        );
      }
    ...
    const setupWindowEthereum = () => {
      ...
      if (!existing) {
        ...
        try {
          ...
          console.warn(
            'Hero Wallet: defineProperty failed, using direct assignment'
          );
          globalAny.ethereum = provider;
        }
